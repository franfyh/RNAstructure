...
# Linux
CXXFLAGS =  -O3 -fsched-spec-load -fPIC -D NDEBUG  -fpermissive 
MULTIFINDFLAG = -DMULTIFIND
PROBCONSFLAG = -DNumInsertStates=1 -DVERSION="1.10"
LIBFLAGS = -shared
OPSYSTEM = Linux
...
# Define an TURBOFOLD compiling rule.
COMPILE_TURBOFOLD = ${CXX} ${CXXFLAGS} -DNumInsertStates=1 -DVERSION="1.10" -c -o $@

# Define an SMP TURBOFOLD compiling rule.
COMPILE_TURBOFOLD_SMP = ${CXX} ${CXXFLAGS}  -DCOMPILE_SMP -DNumInsertStates=1 -DVERSION="1.10" -c -o $@
...
COMPILE_SVM = ${CXX} ${CXXFLAGS} -I${ROOTPATH}/libsvm-3.18/ -c -o $@

# Define an SVM/SMP compiling rule.
COMPILE_SVM_SMP = ${CXX} ${CXXFLAGS} -I${ROOTPATH}/libsvm-3.18/ -DCOMPILE_SMP -c -o $@
