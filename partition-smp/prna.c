...
DEV HOST static real_t *array_val(real_t *a, int i, int j, int n, const base_t *s)
{
...
(int d, int n, base_t *s, real_t *v, real_t *x, real_t *w5, real_t *w3, param_t p)
{
  int i;
...
(int d, int n, base_t *s, real_t *v, param_t p)
{
  int i;
  for (i = ISTART; i < n; i += IINC) {
    const int jtmp = i+d+1;
...
#define ALLOC(a,sz) CU(cudaMalloc(&a,(sz)*sizeof(real_t)))
  ...
  for (d = 0; d < n-1; d++) {
    ...
    CU(cudaEventRecord(start,0));
    calc_hairpin_stack_exterior_multibranch<<<n,1>>>(d, n, dev_s, v, x, w5, w3, dev_par);
    CU(cudaEventRecord(stop,0));
    CU(cudaEventSynchronize(stop));
    CU(cudaEventElapsedTime(&t,start,stop));
    t1 += t;
    ...
    CU(cudaEventRecord(start,0));
    calc_internal<<<n,dim3(THREAD_X,THREAD_Y,1)>>>(d, n, dev_s, v, dev_par);
    CU(cudaEventRecord(stop,0));
    CU(cudaEventSynchronize(stop));
    CU(cudaEventElapsedTime(&t,start,stop));
    t2 += t;
    ...
    CU(cudaEventRecord(start,0));
#ifndef DISABLE_COAXIAL
    calc_coaxial<<<n,NTHREAD>>>(d, n, dev_s, v, y, w5, w3, dev_par);
#endif
    CU(cudaEventRecord(stop,0));
    CU(cudaEventSynchronize(stop));
    CU(cudaEventElapsedTime(&t,start,stop));
    ...
    CU(cudaEventRecord(start,0));
    calc_wl<<<n,1>>>(d, n, dev_s, v, z, wq, w, wl, dev_par);
    CU(cudaEventRecord(stop,0));
    CU(cudaEventSynchronize(stop));
    CU(cudaEventElapsedTime(&t,start,stop));
    t4 += t;
    ...
    CU(cudaEventRecord(start,0));
#ifndef DISABLE_COAXIAL
    calc_z<<<n,NTHREAD>>>(d, n, dev_s, v, z, xl, wq, dev_par);
#endif
    CU(cudaEventRecord(stop,0));
    CU(cudaEventSynchronize(stop));
    CU(cudaEventElapsedTime(&t,start,stop));
  ...
  fprintf(stderr, "Time for hairpin, stack, exterior, multibranch: %3.1f ms\n", t1);
  fprintf(stderr, "Time for internal: %3.1f ms\n", t2);
  fprintf(stderr, "Time for coaxial: %3.1f ms\n", t3);
  fprintf(stderr, "Time for calc_wl: %3.1f ms\n", t4);
  fprintf(stderr, "Time for calc_xl: %3.1f ms\n", t5);
  fprintf(stderr, "Time for calc_z: %3.1f ms\n", t6);
  fprintf(stderr, "Time for calc_x: %3.1f ms\n", t7);
  fprintf(stderr, "Time for w3, w5: %3.1f ms\n", t8);
...
#define ALLOC(a,sz) a = (real_t *) safe_malloc((sz)*sizeof(real_t))
  ...
  int d;
  for (d = 0; d < n-1; d++) {
    calc_hairpin_stack_exterior_multibranch(d, n, p->seq, p->v, x, p->w5, p->w3, par);
    calc_internal(d, n, p->seq, p->v, par);
#ifndef DISABLE_COAXIAL
    calc_coaxial(d, n, p->seq, p->v, y, p->w5, p->w3, par);
#endif
    calc_wl(d, n, p->seq, p->v, z, wq, w, wl, par);
    calc_xl(d, n, z, yl, xl);
#ifndef DISABLE_COAXIAL
    calc_z(d, n, p->seq, p->v, z, xl, wq, par);
#endif
...
#define SHOWARR(a)	 \
  if (p->a) {		      \
    ...
    for (i = 0; i < n; i++) { \
      printf("%s%4d: ",#a,i+1);				\
      for (j = 0; j < n; j++) {				\
	const real_t *aij = array_val(p->a,i,j,n,s);		\
...
static real_t probability_of_pair(const prna_t p, int i, int j)
{
