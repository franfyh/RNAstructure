...
void efn2Interface::run() {
	...
	if( error == 0 ) {
		...
		// Write a simple list file, if asked.
			...
			strand->CalculateFreeEnergy( 1 );			//calculate the free energy of the first structure first (this is to make it play well with openMP)			
			#ifdef SMP
			#pragma omp parallel for
			#endif
			for( int i = 2; i <= structures; i++ ) { 
				strand->CalculateFreeEnergy( i ); 		//now calculate the free energy of the remaining structures
				error = checker->isErrorStatus();
