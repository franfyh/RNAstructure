...
void CDrawView::DetermineColor() {
	...
	double probability;
		...
		for (i=1;i<=pDoc->ct.numofbases;i++) {
			...
			if (pDoc->ct.basepr[pDoc->StructureNumber][i]==0) {
				//pDoc->color[i]=0;
				...
				for (j=i+1;j<=pDoc->ct.numofbases;j++) probability-= (double) calculateprobability(i,j,v,w5,&pDoc->ct,data,lfce,mod,scaling,fce);	
			}
			 
			else if (pDoc->ct.basepr[pDoc->StructureNumber][i]>i) probability =(double) calculateprobability(i,pDoc->ct.basepr[pDoc->StructureNumber][i],v,w5,&pDoc->ct,data,lfce,mod,scaling,fce);
			else probability =(double) calculateprobability(pDoc->ct.basepr[pDoc->StructureNumber][i],i,v,w5,&pDoc->ct,data,lfce,mod,scaling,fce);
