	...
	for(unsigned int pknum=0;pknum<pk.size();++pknum){
		...
		while(b<e){
			...
			//start determining what size of spanning helix it is.
				...
				//else cerr << endl << "                                 KISSING LOOP"<< endl;
				SHLrepeatBOOL=false;
...
void ReadPseudoParam(double *pseudo_param)
{
	//Read datapath.
	string fn("pseudconst.dat");//Holds the file name with pseudoknot penalty calculation constants.
	string dp(".");//Holds the path to data_tables.
	if(getenv("DATAPATH")!=NULL){//If Datapath is not NULL...
		dp=string(getenv("DATAPATH"));//...set 'dp' to hold "DATAPATH".
	}
	...
	if(!pseudoConst.is_open()){
		cerr << "File \"pseudconst.dat\" with pseudoknot penalty constants is not found. This file has to be placed in \"~/RNAstructure/data_tables/\"." << endl;
		cerr << "Please check file and DATAPATH. DATAPATH should be set to \"~/RNAstructure/data_tables/\" " << endl;
	}
	...
	for(int i=0;i<=15;++i){
		...
		while(strcmp(lineoftext.c_str(),"-->")) pseudoConst>>lineoftext;
		//When '-->' is reached, store it in the constant.
		pseudoConst>>pseudo_param[i];
  ...
  double energy=0;//store energy in 'energy'
    for(int i=1;i<=ct->GetSequenceLength()-7;++i){//for every nucleotide up to 7 nucleotides till the end of the sequence
        if(i<ct->GetPair(i,strnum)&&ct->GetPair(i,strnum)!=0){//if it is a 5' to 3' pair
            //If there is a stack
            //cerr << "\nCHECK1   i=" << i << " i+1=" << i+1 << " ct->basepr[" << strnum << "][" << i << "]-1=" << (ct->basepr[strnum][i]-1) << endl;
            if(ct->GetPair(i+1,strnum)==ct->GetPair(i,strnum)-1){
                //get the energy of the stack
                energy+=erg1(i, ct->GetStructure()->GetPair(i,strnum), i+1, ct->GetStructure()->GetPair(i,strnum)-1, ct->GetStructure(), data);
                //cerr << '\n' << "STACK i=" << i << " j=" << ct->basepr[strnum][i] << " iP=" << i+1 << " jP=" << ct->basepr[strnum][i]-1 << " erg1=" << erg1(i, ct->basepr[strnum][i], i+1, ct->basepr[strnum][i]-1, ct, data) << '\n';
            }
            //If there is a bulge or internal loop
                ...
                if(i+1 < ct->GetPair(i+1,strnum)){
                    ...
                    if((Pi-Ci)<3&&(Cj-Pj)<3){
                        energy+=erg2(Ci, Cj, Pi, Pj, ct->GetStructure(), data, 0, 0);
                        //cerr << '\n' << "BULGE/LOOP i=" << Ci << " j=" << Cj << " iP=" << Pi << " jP=" << Pj << " erg1=" << erg2(Ci, Cj, Pi, Pj, ct, data, 0, 0) << '\n';
                    }
                }
            }
...
double CalculatePseudoFreeEnergy(RNA* rna, int strnum, double P1, double P2, double* pseud_param, datatable* data, bool UseSimpleMBLoopRules, bool fillMismatch, bool remIsolatedPairs)
{
    ...
    if(fillMismatch) FillMismatch(rna,strnum);

    //If the 'remIsolatedPairs' option was specified, proceed
    if(remIsolatedPairs) RemoveIsolatedPairs(rna,strnum);

    //Check if there is still a pseudoknot in the 'rep' structure
    if(rna->ContainsPseudoknot(strnum)){
    
        //Calculate the pseudoknot penalty
        int PseudPenalty=pkEnergyCalc(rna, pseud_param, P1, P2, strnum);
