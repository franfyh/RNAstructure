...
void t_phmm_aln::set_indel_priors(vector<double>* _indel_priors)
{
	...
	// Size check?
//        cerr << "height " << _aln_priors->height << " width " << _aln_priors->height << "\n"; 
	this->indel_priors = new vector<double>(*_indel_priors);
        //        cerr << this->indel_priors->size() << "\n";
...
double t_phmm_aln::get_match_prior(int i, int k, int n1, int n2)
{
	...
	if(this->match_priors != NULL)
	{
		...
		return(xlog(1+this->match_priors->x(i,k)));
	}
	else
	{
...
double t_phmm_aln::get_indel_prior(int i, int k, int n1, int n2)
{
        if(this->indel_priors == NULL) return(0.0f);
        if(i == 0) 
        { 
                 if(k == 0 || k == n2+1)
                 {
                 ...
                 {
                     return(xlog(this->indel_priors->at(k + n1) ) );
                 }
        }
        else
        {
                 ...
                 {
                     return(xlog(this->indel_priors->at(i) ) );
                 }
        }
}
