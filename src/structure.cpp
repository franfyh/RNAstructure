...
//        singlestranded_constant.assign(numofbases+1, 1);
//        singlestranded_cumulative_log_constant.assign(numofbases+1, 0);
//<<<<<<< structure.cpp
...
//            delete singlestranded_constant;

        //          delete singlestranded_cumulative_log_constant;
...
//        singlestranded_constant.assign(numofbases+1, vector<double>(numofbases+1,1));
        singlestranded_constant.push_back(vector<double>());
        
        for(int i = 1;i <= numofbases;++i){
            singlestranded_constant.push_back(vector<double>(numofbases+1-i,1));
        }
...
/*
void allocate_singlestranded_info()

{
    singlestranded_constant = new vector<double>(numofbases+1, 1);
    singlestranded_cumulative_log_constant = new vector<double>(numofbases+1, 0);

}
*/

double structure::singlestrandedconstant(int i)

{
    if(i <= numofbases)
        return singlestranded_constant[i][0];
    else
        return singlestranded_constant[i-numofbases][0];
}

double structure::singlestrandedconstant(int i, int j)

{
    if(j >= i) 
    {
        if(i <= numofbases){
            return singlestranded_constant[i][j-i];
        }
        else
            return singlestranded_constant[i-numofbases][j-i];
    }
    else
        return 1;
}

int structure::initialize_singlestranded()
{
    for(int k = 1; k <= numofbases; k++)
    {
        for(int l = 0; l < singlestranded_constant[k].size(); l++){
            singlestranded_constant[k][l] = 1;
        }
    }
}

int structure::set_singlestranded(vector<double>* value)
{

    for(int d = 1; d <= numofbases; d++){
        for(int i = 1; i <= numofbases; i++){
            int j = i+d-1;
            if(j>numofbases)break;
//            cerr << "i "<<i<<" j "<<j<<"\n";
            if(d==1)
                singlestranded_constant[i][d-1]=value->at(i);
            else{
                singlestranded_constant[i][d-1]=singlestranded_constant[i][d-2]*value->at(j);
            }
        }
    }
}
