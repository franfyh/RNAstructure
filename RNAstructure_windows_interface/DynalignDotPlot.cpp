...
void CDynalignDotPlot::OnBnClickedOk()
{
	...
	for (i=lowlimit(ct1.numofbases,maxsep,ct1.numofbases,ct2.numofbases);i</*2*maxsep+2*/highlimit(ct1.numofbases,maxsep,ct1.numofbases,ct2.numofbases)&&i<=ct2.numofbases;i++) {
		
			//k = i+ct1.numofbases-maxsep;
			//if (k<=ct2.numofbases) {
				if (w5->f(ct1.numofbases,i)/*[ct1.numofbases][i]*/+gap*(ct2.numofbases-i)/*(i-maxsep)*/<crit) crit = w5->f(ct1.numofbases,i)/*[ct1.numofbases][i]*/+gap*(ct2.numofbases-i);	
			//}
	...
	for (i=1;i<=ct1.numofbases;i++) {
		for (j=i+minloop;j<=ct1.numofbases;j++) {
			for (k=max(/*i-maxsep*/lowlimit(i,maxsep,ct1.numofbases,ct2.numofbases),1);k<=(min(ct2.numofbases,highlimit(i,maxsep,ct1.numofbases,ct2.numofbases)/*i+maxsep*/));k++) {
				for (l=max(/*j-maxsep*/lowlimit(j,maxsep,ct1.numofbases,ct2.numofbases),k);l<=(min(ct2.numofbases,highlimit(j,maxsep,ct1.numofbases,ct2.numofbases)/*j+maxsep*/));l++) {
					...
					//if ((a+ct2.numofbases-ct1.numofbases>=0)&&(a+ct2.numofbases-ct1.numofbases<=2*maxsep)) {
						if (v->f(i,j,k,l)/*v[j][i][a][b]*/+v->f(j,i+ct1.numofbases,l,k+ct2.numofbases)/*v[i+ct1.numofbases][j-i][b][a]*/<=crit) 
							heap.push(i,j,k,l,v->f(i,j,k,l)/*v[j][i][a][b]*/+v->f(j,i+ct1.numofbases,l,k+ct2.numofbases));
					//}
